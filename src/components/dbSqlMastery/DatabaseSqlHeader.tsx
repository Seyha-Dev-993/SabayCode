...
import { ChevronLeft, Database, CheckCircle2 } from 'lucide-react';
...
export const DatabaseSqlHeader: React.FC<Props> = ({
  enrollment,
  currentLessonTitle,
  onNavigateOverview,
  onBackToClasses,
  ...
  const totalLessons = getAllDatabaseSqlLessons().length;
    ...
    <header className="sticky top-0 z-40 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 shadow-sm dark:shadow-lg transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          >
            <div className="p-1.5 rounded-lg bg-gradient-to-br from-indigo-600 via-blue-600 to-cyan-500 text-white shadow-md shadow-blue-500/20 shrink-0 flex items-center justify-center">
              <Database className="w-4 h-4" />
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <span className="font-bold text-xs sm:text-sm text-slate-900 dark:text-slate-100 truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                  {DATABASE_SQL_CLASS_INFO.title[lang]}
                </span>
                <span className="hidden md:inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-100 dark:bg-blue-950/80 text-blue-800 dark:text-blue-300 border border-blue-300 dark:border-blue-800/60">
                  SQL + RDBMS
                </span>
              </div>
              {currentLessonTitle && (
