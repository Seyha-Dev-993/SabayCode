...
import { 
  Server, Search, BookOpen, Layers, 
  ChevronRight, ShieldCheck, Star, Database, Lock, Terminal
} from 'lucide-react';
import { useApp } from '../../context/AppContext';
import { BACKEND_CATEGORIES, ALL_BACKEND_COURSES, BackendCourse } from '../../data/backendCoursesHubData';
import { CourseBrandIcon, getCourseBannerStyle } from '../CourseBrandIcon';

interface BackendHubViewProps {
  onSelectCourse: (courseId: string) => void;
  onNavigateHome: () => void;
}

export const BackendHubView: React.FC<BackendHubViewProps> = ({
  onSelectCourse,
  onNavigateHome,
}) => {
  const { language } = useApp();
  const [searchQuery, setSearchQuery] = useState('');
  const [activeCategory, setActiveCategory] = useState<'all' | 'languages' | 'frameworks' | 'databases' | 'security'>('all');

  // Filter backend courses
  const filteredCourses = ALL_BACKEND_COURSES.filter((course) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10 font-sans text-slate-900 dark:text-slate-100 animate-fade-in">
      ...
      <div className="relative bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 rounded-3xl p-8 sm:p-12 border border-slate-800/90 shadow-2xl overflow-hidden text-white">
        ...
        <div className="relative z-10 max-w-3xl space-y-5">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-500/20 border border-emerald-400/30 text-emerald-300 text-xs font-mono font-bold uppercase tracking-wider">
            <Server className="w-4 h-4 text-emerald-400" />
            <span>{language === 'en' ? 'Backend Engineering Hub' : 'មជ្ឈមណ្ឌល Backend Engineering'}</span>
          </div>

          <h1 className="text-3xl sm:text-5xl font-black tracking-tight text-white leading-tight">
            {language === 'en' ? 'Master Production REST APIs & Enterprise Cloud Backends' : 'រៀនសរសេរ Server, REST APIs & Databases អាជីព'}
          </h1>

          <p className="text-slate-300 text-sm sm:text-base leading-relaxed">
            {language === 'en'
              ? 'Choose a core language, database, framework, or security track to build real-world backend microservices. Independent courses for PHP, SQL, Node.js, Python, Java, Laravel, Express, Django, Spring Boot, MongoDB, and API Security.'
              : 'ជ្រើសរើសភាសាមូលដ្ឋាន, Database, Framework ឬ Security សម្រាប់បង្កើត Web Server & APIs។ មេរៀននីមួយៗដាច់ដោយឡែកពីគ្នាជាមួយគម្រោងអនុវត្តផ្ទាល់។'}
          </p>
          ...
          <div className="pt-2 flex flex-wrap items-center gap-6 text-xs font-mono text-slate-300">
            <div className="flex items-center gap-2">
              ...
              <span>11 {language === 'en' ? 'Independent Courses' : 'វគ្គសិក្សាដាច់ដោយឡែក'}</span>
            </div>
            <div className="flex items-center gap-2">
              <Layers className="w-4 h-4 text-blue-400" />
              <span>{language === 'en' ? 'Core Languages vs Frameworks & DBs' : 'ភាសាមូលដ្ឋាន, Frameworks & DBs'}</span>
            </div>
            <div className="flex items-center gap-2">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
        {/* Category Pills */}
        <div className="flex flex-wrap items-center gap-2 p-1.5 bg-slate-100 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 w-full md:w-auto">
          <button
          ...
          >
            {language === 'en' ? 'All 11 Courses' : 'វគ្គសិក្សាទាំងអស់ (១១)'}
          </button>
          <button
          ...
          >
            {language === 'en' ? '3. DBs & Security' : '៣. DBs & សុវត្ថិភាព'}
          </button>
        </div>
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            placeholder={language === 'en' ? 'Search SQL, Laravel, Express...' : 'ស្វែងរក SQL, Laravel, Express...'}
            className="w-full pl-10 pr-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-xs text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 shadow-sm"
          />
        </div>
      </div>
      ...
      {BACKEND_CATEGORIES.map((cat) => {
        if (activeCategory !== 'all' && activeCategory !== cat.id && !(activeCategory === 'databases' && (cat.id === 'databases' || cat.id === 'security'))) return null;
